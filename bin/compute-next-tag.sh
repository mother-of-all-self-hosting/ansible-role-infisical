...
#
# Tags look like `v<Infisical version>-<release>`:
#
# - if defaults/main.yml points at an Infisical version that has never been
#   released, the release counter restarts at 0 (`v0.162.21-0`)
# - otherwise the counter is incremented (`v0.162.21-1`), but only if something
...
version="$(sed -nE 's|^infisical_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Infisical version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v0.162.21`),
